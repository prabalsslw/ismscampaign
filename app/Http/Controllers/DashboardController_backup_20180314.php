...
class DashboardController extends Controller
{
    ...
    public function CampaignLeftBar()
    {
        ...
        if(Session::has('user_id'))
        {
            ...
            ->leftJoin('sms_status', function($join)
            {
                $join->on('sms_status_tb.sms_status', '=', 'sms_status.id');
            })
    ...
    public function SummarySearch(Request $request)
    {
        ...
        {
            $isms_summary = DB::select("SELECT * FROM isms_summary LEFT JOIN sms_status_tb ON isms_summary.session_id = sms_status_tb.sms_session_id LEFT JOIN stake_holder_tb ON stake_holder_tb.id = sms_status_tb.stakeholder_id LEFT JOIN sms_status ON sms_status_tb.sms_status = sms_status.id WHERE sms_status_tb.send_user_id = '$user_id' and sms_status_tb.campaign_type = '2' and isms_summary.upload_date BETWEEN '$date_from' AND '$date_to' and stake_holder_tb.stake_holder = '$stakeholder' ");
            return view('smsstatus', compact('sql','isms_summary'));
        }

        if (!empty($date_from) && !empty($date_to))
        {
            $isms_summary = DB::select("SELECT * FROM isms_summary LEFT JOIN sms_status_tb ON isms_summary.session_id = sms_status_tb.sms_session_id LEFT JOIN stake_holder_tb ON stake_holder_tb.id = sms_status_tb.stakeholder_id LEFT JOIN sms_status ON sms_status_tb.sms_status = sms_status.id WHERE sms_status_tb.send_user_id = '$user_id' and sms_status_tb.campaign_type = '2' and isms_summary.upload_date BETWEEN '$date_from' AND '$date_to' ");
            return view('smsstatus', compact('sql','isms_summary'));
        }

        if (!empty($stakeholder)) 
        {
            $isms_summary = DB::select("SELECT * FROM isms_summary LEFT JOIN sms_status_tb ON isms_summary.session_id = sms_status_tb.sms_session_id LEFT JOIN stake_holder_tb ON stake_holder_tb.id = sms_status_tb.stakeholder_id LEFT JOIN sms_status ON sms_status_tb.sms_status = sms_status.id WHERE sms_status_tb.send_user_id = '$user_id' and sms_status_tb.campaign_type = '2' and stake_holder_tb.stake_holder = '$stakeholder' ");
            return view('smsstatus', compact('sql','isms_summary'));
        }
